...
// qualifying event to one script at chance 100, and this table says which spells the
// talent answers to. An empty list means any spell the flags already allow.
...
{
    std::uint32_t Talent;
    std::array<std::uint32_t, 20> Spells;   // zero terminated; all zero means "anything"
};

// The spell ids are every rank of the ability the tooltip names.
inline constexpr std::array<Rule, 14> Rules =
{{
